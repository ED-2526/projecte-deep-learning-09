...
class Config:
    ...
    cache_file: str = "features_cache_baseline.pkl"
    project_name: str = "image-captioning"
    run_name: str = "wordlevel-baseline"
    max_images: int = 8000
    test_size: float = 0.2
    random_state: int = 42
    batch_size: int = 32
...
def generate_caption_greedy(
    ...
    with torch.no_grad():
        ...
        for _ in range(max_length):
            ...
            next_token = decoder.fc(output.squeeze(1)).argmax(1)
            predicted_word = tokenizer.convert_ids_to_tokens(next_token.item())
...
def main() -> None:
    ...
    train_images, val_images = train_test_split(
        [image_name for image_name in image_names if image_name in features_dict],
    ...
    encoder = CNNEncoder(config.embed_size).to(config.device)
    decoder = RNNDecoder(
